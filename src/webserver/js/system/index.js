    ...
    if (sMeta_source == "customer") { /** 客户消息 */
	    ...
	    if ($("#" + sHeaderId).length == 0) {
	    	...
	    	var sContent = sKh_nc + "[" + sKh_xbmc + "/" + sKh_cs + "/" + sKh_yy + "]"
	    	             ...
	    	             + " kh_yy='" + sKh_yy + "'"
	    		         + ">" + "[进入对话]" + "</span>";*/
	    	var sContent = "<br><span style='cursor:pointer' onclick='javascript:showCustomerTab(this)'"
	    				 + " kh_khbh='" + sKh_khbh + "'"
	    	             + " kh_khzh='" + sKh_khzh + "'"
	    	             + " kh_openid='" + sKh_openid + "'"
	    	             + " kh_nc='" + sKh_nc + "'"
	    	             ...
	    	             + " kh_cs='" + sKh_cs + "'"
	    	             + " kh_yy='" + sKh_yy + "'>"
	    		         + " <div style='text-align:left;'><span><img src='http://weixin.bxhg.net/Aimee/resources/image/index/webwxgeticon.jpg' style=’width:46px;height:46px;vertical-align:middle;'></span>" + sKh_nc 
	    		         + " &nbsp"+sMsg_sendtime.substr(11,8)+"</div></span>";	         
	    	addAccordionHeader(sHeaderId, sContent);
	    }
...
function showCustomerTab(sCustomerInfo) {
	...
	if (cnt == 0) {  /** TAB不存在 */
		...
		var sDialogueWordsArea = '<div id="dialogueWordsArea_' + sKh_khbh + '" readonly="readonly" rows="18" cols="20" style="width:100%;height:380px;BORDER-BOTTOM:0px solid;BORDER-LEFT:0px solid;   BORDER-RIGHT:0px solid;BORDER-TOP:0px solid;overflow-y:auto;overflow-x:hidden;border:0;"></div>';
		var sInputWordsArea = "<form id='inputWordsForm_" + sKh_khbh + "' method='post' enctype='multipart/form-data'>"
		                    + $("#inputWordsFormTamplate").html()
		                    + "</form>";
		var sContent = "<table border='1' width='100%'><tr><td>"
			        + sDialogueWordsArea
			        + "</td><td valign='top' rowspan='2' width='100'><b>使用技巧或其它动态消息</b>" 
			        + "<br>具体内容等...</td>"
			        + "</tr><tr><td>"
			        + sInputWordsArea
			        +"</td></tr></table>";
...
function sendMsgToCustomer2() {
	...
	//alert(thistab.html()); 
//alert(thistab.panel('options').khbh);
	...
	/** 向客户发消息 */
	var bSuccess = sendTextMsgToCustomer(sKh_khbh, sKh_openid, sMsg_content);
	//if (bSuccess) {
...
function addTextMsgToDialogueWordsArea(sMsgSender, sMsg_content, sKhbh, sNowStr) {
	...
	var e=document.getElementById("dialogueWordsArea_" + sKhbh);
...
function uploadImageFile() {
	...
	$('#inputWordsForm_' + sKh_khbh).ajaxSubmit({
		 type: "post",
	     url: sCtx + "/widgets/attachment/fileUploadProcess.do",
	     dataType: "json",
